...
/* Add the map { kernel[id] -> type[sizes] } to gen->sizes,
 * if the option debug->dump_sizes is set.
 */
static void set_sizes(struct gpu_gen *gen, const char *type, int id,
	int *sizes, int len)
{
	int i;
	isl_space *space;
	...
	space = isl_union_map_get_space(gen->sizes);	
	...
	gen->sizes = isl_union_map_add_map(gen->sizes, map);
}
...
/* Is the array "array" read only. */
static int is_read_only(struct gpu_array_info *array,
	...
	struct gpu_prog *prog)
{
	...
	// it is a scalar but is it read only?
	return is_read_only(array, prog);
}
	...
	__isl_keep isl_union_set *arrays)
{
	...
	info->read_only = is_read_only(info, prog);
	info->texture = 0;
	...
	int *sizes, int len)
{
	...
	//set = isl_set_fix_si(set, isl_dim_set, 0, id);
	//isl_set_dump(set);
	//exit(0);
	// END EXPERIMENT
	space = isl_space_from_domain(space);
	...
	__isl_keep isl_id *id, void *user)
{
	...
	if (data->array->texture) {
	  ...
	  //isl_ast_expr *name = isl_ast_expr_get_op_arg(expr, 0);
	  isl_ast_expr_list *args = isl_ast_expr_list_alloc(ctx, n_args);

	  char texName[50];
	  if (data->array->n_index == 1) {
	    strcpy(texName, "tex1D<");
	  }
	  else if (data->array->n_index == 2) {
	    strcpy(texName, "tex2D<");	   
	  }
	  else {
	    strcpy(texName, "tex3D<");
	  }
	  strcat(texName, data->array->type);
	  strcat(texName, ">");

	  isl_id *tid = isl_id_alloc(ctx, texName, NULL);
	  isl_ast_expr *tex = isl_ast_expr_from_id(tid);
	  
	  isl_ast_expr *arg0 = isl_ast_expr_get_op_arg(expr, 0);
	  isl_ast_expr_list_add(args, arg0);
	  // add the arguments to the list
	  for (i = 1; i < n_args; ++i) {
	    isl_ast_expr *arg;
	    
	    arg = isl_ast_expr_get_op_arg(expr, n_args - i);
	    isl_ast_expr_list_add(args, arg);
	  }

	  isl_ast_expr *texCall = isl_ast_expr_call(tex, args);	  
	  return texCall;
	}
	...
	__isl_keep isl_ast_build *build)
{
	...
	stmt->type = ppcg_kernel_copy;
        ...
        stmt->reduction = (isl_union_set_contains(kernel->reduction_arrays, 
						  group->array->space) > 0);

	stmt->atomic_store = kernel->atomic_store;
...
/* Return the set of array elements written to as a part of the 
 * reduction operation.
 */
static __isl_give isl_union_set *accessed_reduction(__isl_take isl_union_set *arrays, 
						    isl_union_set *domain,
						    struct gpu_prog *prog) {
  int i;
  isl_union_set *reduction_arrays = NULL;
  for (i = 0; i < prog->n_stmts; ++i) {
    struct gpu_stmt *stmt = &prog->stmts[i];
    isl_space *space = isl_set_get_space(stmt->stmt->domain);

    if (!stmt->reduction) 
      continue;

    if (!isl_union_set_contains(domain, space)) {
      // statement not in the kernel
      continue;
    }

    struct gpu_stmt_access *access = stmt->accesses;
    
    // traverse accesses to look for writes
    while (access != NULL) {
      if (access->write) {
	isl_union_map *arr_map;
	arr_map = isl_union_map_intersect_range(isl_union_map_from_map(
						   isl_map_copy(access->access)), 
						isl_union_set_copy(arrays));
	if (reduction_arrays == NULL)
	  reduction_arrays = isl_union_map_range(arr_map);
	else 
	  reduction_arrays = isl_union_set_union(reduction_arrays, isl_union_map_range(arr_map));
      }
      access = access->next;
    }
  } // end for i
 ...
 * by the statement instance in "domain" in "prog".
 */
static __isl_give isl_union_set *accessed_by_domain_write(
	__isl_take isl_union_set *domain, struct gpu_prog *prog)
{
	isl_union_map *access;
	isl_union_set *arrays;
	printf("accessed_by_domain_write\n");
	isl_union_set_dump(domain);
	printf("a1\n");
	access = isl_union_map_copy(prog->may_write);
	printf("a2\n");
	...
	printf("end accessed_by_domain_write\n");
	return arrays;
}
	...
	struct gpu_array_ref_group *group, int read)
{
	...
	// BEGIN EXPERIMENT
	/*space = isl_space_drop_dims(space, isl_dim_out, 0, 1);	
...
static isl_stat set_gpu_reduction(__isl_take isl_set *set, void *user) {
  struct gpu_prog *prog = (struct gpu_prog *)user;
  isl_id *id = isl_set_get_tuple_id(set);

  struct gpu_stmt *stmt = find_stmt(prog, id);

  assert(stmt != NULL);
  assert (pet_tree_get_type(stmt->stmt->body) == pet_tree_expr);
  pet_expr *expr = pet_tree_expr_get_expr(stmt->stmt->body);
  pet_expr *call = pet_expr_new_call(prog->ctx, "reduce", 2);
  
  call = pet_expr_set_arg(call, 0, expr->args[pet_bin_lhs]);
  call = pet_expr_set_arg(call, 1, expr->args[pet_bin_rhs]);
  
  // can call a macro to do the reduction  
  stmt->stmt->body = pet_tree_new_expr(call);
  //isl_union_map *map = pet_expr_access_get_access(expr->args[pet_bin_lhs], pet_expr_double);
  //isl_union_map_dump(map);
  //pet_function_summary_dump(summ);
...
/* A isl_schedule_foreach_schedule_node_top_down callback
 * for setting the reduction flag in gpu_stmts and aborting 
 * the search if "node" has any reduction flags set. 
 * Otherwise, continue searching.
 */
static isl_bool mark_reduction(__isl_keep isl_schedule_node *node, void *user) {
  int i;
  if (isl_schedule_node_get_type(node) != isl_schedule_node_band) {
    // abort, no reductions
    return isl_bool_error;
  ...
  const unsigned n = isl_schedule_node_band_n_member(node);

  struct gpu_prog *prog = (struct gpu_prog *)user;

  for (i = 0; i < n; ++i) {
    if (isl_schedule_node_band_member_get_reduction(node, i)) {
      isl_union_set *domain = isl_schedule_node_get_domain(node);
      isl_union_set_foreach_set(domain, &set_gpu_reduction, user);
      return isl_bool_error;
    }
  }
...
/* Set the reduction flags of gpu_stmt in prog 
 * if any of the bands in 'node' are set to reduction.
 */
static void mark_reduction_stmts(__isl_keep isl_schedule_node *node, struct gpu_prog *prog) {
  isl_schedule_node_foreach_descendant_top_down(node, &mark_reduction, prog);
}
...
static isl_stat get_stmt_str(__isl_take isl_set *set, void *user) {
  char *str = (char *)user;
  strcat(str, strdup(isl_set_get_tuple_name(set)));
  strcat(str, ",");
  return isl_stat_ok;
}
	...
	__isl_keep isl_multi_val *sizes)
{
	...
	printf("d5.1\n");
	isl_union_set_dump(domain);
	isl_union_set *write_arrays = accessed_by_domain_write(isl_union_set_copy(domain),
							       gen->prog);
	printf("d6\n");
	mark_reduction_stmts(node, kernel->prog);

	kernel->reduction_arrays = accessed_reduction(isl_union_set_copy(write_arrays), 
						      domain,
						      gen->prog);
	printf("d7\n");
	kernel->atomic_store = 0;
	if (gen->wrap) {
	  struct ca_wrapper *wrap = gen->wrap;
	  struct ca_ant *ant = wrap->ant;
	  
	  // store the kernel stmt string array
	  printf("kernel id %d, %d\n", gen->kernel_id, ant->kernel_stmt_id);
	  assert(gen->kernel_id == ant->kernel_stmt_id);
	  int kid = gen->kernel_id;
	  printf("d1\n");
	  ++ant->kernel_stmt_id;
	  if (wrap->ant->kernel_stmts == NULL) {
	    ant->kernel_stmts = (char ***)calloc(1, sizeof(char **));
	    ant->kernel_stmts_len = (int *)calloc(1, sizeof(int));
	    printf("d2\n");
	  }
	  else {
	    printf("d3\n");
	    ant->kernel_stmts = (char ***)realloc(ant->kernel_stmts, 
						  ant->kernel_stmt_id * sizeof(char **));

	    ant->kernel_stmts_len = (int *)realloc(ant->kernel_stmts_len, 
						   ant->kernel_stmt_id * sizeof(int));
	    ant->kernel_stmts_len[ant->kernel_stmt_id - 1] = 0;
	    printf("d4\n");
	  }
	  
	  printf("d5\n");
	  ant->kernel_stmts_len[kid] = isl_union_set_n_set(kernel->core);
	  assert (ant->kernel_stmts_len[kid] > 0);
	  printf("d6\n");

	  ant->kernel_stmts[kid] = (char **)calloc(ant->kernel_stmts_len[kid], sizeof(char *));
	  struct stmt_arr arr;
	  arr.stmt_str = ant->kernel_stmts[kid];
	  arr.len = 0;
	...
	if (gen->wrap) {
	  // optimize for shared memory
	  struct ca_wrapper *wrap = gen->wrap;
	  char *key = (char *)calloc(500, sizeof(char));
	  snprintf(key, 500, "Shared-%d:", kernel->n_block);
	  isl_union_set_foreach_set(kernel->core, &get_stmt_str, key);

	  kernel->options->use_shared_memory = wrap->traverse_shared_mem(wrap->ant, 
									 wrap->s_path, 
									 wrap->node, key);
	  ...
	  assert(node != NULL);
	  assert(s_path->next == NULL);

	  wrap->s_path = s_path;
	  wrap->node = node;    
	  
	  kernel->options->cuda_cache_config = wrap->traverse_cache_config(wrap->ant, 
						       wrap->s_path, 
						       wrap->node, 
						       kernel->options->use_shared_memory);
	  ...
	  char *atomic_key = (char *)calloc(500, sizeof(char));
	  snprintf(atomic_key, 500, "Atomic-%d:", kernel->n_block);
	  isl_union_set_foreach_set(kernel->core, &get_stmt_str, atomic_key);

	  // TRAVERSE ATOMIC STORE
	  kernel->atomic_store = wrap->traverse_atomic_store(wrap->ant, wrap->s_path, 
							     wrap->node, atomic_key);
	  if (kernel->atomic_store) {
	    printf("using atomic store\n");
	  }
	  else {
	    printf("NOT using atomic store\n");
	  }
...
/* Strip-mine node by unroll_factors and mark the strip-mined node for 
 * unrolling. Return a pointer to the strip-mined node.
 */
static __isl_give isl_schedule_node *strip_mine_and_unroll(
	__isl_take isl_schedule_node *node, isl_multi_val *unroll_factors) {

  	node = isl_schedule_node_band_tile(node, unroll_factors);
	node = isl_schedule_node_child(node, 0);
...
struct est_regs_helper {
  struct gpu_prog *prog;
  int nregs;
};

static isl_stat estimate_registers(__isl_take isl_set *set, void *user) {
  struct est_regs_helper *helper = (struct est_regs_helper *)user;
  isl_id *id = isl_set_get_tuple_id(set);
  
  struct gpu_stmt *stmt = find_stmt(helper->prog, id);
  struct gpu_stmt_access *trav = stmt->accesses;
...
static int get_n_registers(isl_schedule_node *node, struct gpu_prog *prog) {
  printf("get_n_registers\n");
  struct est_regs_helper helper;
  helper.prog = prog;
  helper.nregs = 0;
  
  isl_union_set *domain = isl_schedule_node_get_domain(node);
  isl_union_set_foreach_set(domain, &estimate_registers, &helper);
  printf("end get_n_registers\n");
  return helper.nregs;
}

static __isl_give isl_multi_val *get_unroll_factors(
	__isl_take isl_schedule_node *node, const int tile_len, 
	const int n_coincident, struct gpu_gen *gen, const int nregs) {
  
  int i;  
  isl_ctx *ctx;
  isl_space *space;
  isl_multi_val *unroll_factors;
  ...
  unroll_factors = isl_multi_val_zero(space);
  
  for (i = 0; i < n_coincident; ++i) {
    unroll_factors = isl_multi_val_set_val(unroll_factors, i, isl_val_one(ctx));
  }
  
  // select unroll factors
  const int n_unroll_factors = (tile_len - n_coincident);
  if (n_unroll_factors == 0) // nothing to unroll
    return NULL;
  
  isl_union_set *domain = isl_schedule_node_get_domain(node);
  int n_stmts = isl_union_set_n_set(domain);
  char **stmts = (char **)calloc(n_stmts, sizeof(char *));

  struct stmt_arr arr;
  arr.stmt_str = stmts;
  arr.len = 0;
  isl_union_set_foreach_set(domain, &get_stmt_str_array, &arr);

  int *unroll = wrap->traverse_unroll(wrap->ant, wrap->s_path, wrap->node, 
				      nregs, n_unroll_factors, stmts, n_stmts);
  
  // update wrap node to point to the current leaf
  struct ca_node *canode = wrap->node;
  struct path *s_path = wrap->s_path;
  while(s_path->next != NULL) {
    assert(canode != NULL);
    assert(canode->out_edges != NULL);
    assert(canode->n_edges > 0);
    printf("%d, %d\n", s_path->v, canode->n_edges);
    assert(s_path->v < canode->n_edges);

    canode = canode->out_edges[s_path->v].dst;
    s_path = s_path->next;

    assert(canode != NULL);
    canode->kernel_id = gen->kernel_id;
    canode->in_kernel = 1;
    assert(s_path != NULL);
  }
  assert(s_path->next == NULL);
  ...
  int unrolled = 0;
  for (i = n_coincident; i < tile_len; ++i) {
    isl_val *v;
    v = isl_val_int_from_si(ctx, unroll[i - n_coincident]);
    unroll_factors = isl_multi_val_set_val(unroll_factors, i, v);

    assert (unroll[i - n_coincident] > 0);

    if (unroll[i - n_coincident] > 1) {
      unrolled = 1;
    }
  }
  printf("end get_unroll_factors\n");

  return (unrolled ? unroll_factors : NULL);  

}

/* Unroll "node" by a number of iterations based on coding ants. Returns 
 * the estimated number of registers needed.
 */
static int unroll_nodes(__isl_keep isl_schedule_node *node, struct gpu_gen *gen) {
  int i;
  int nregs = 0;
  printf("unroll_nodes\n");
  if (isl_schedule_node_n_children(node) == 0) {
    ...
    return get_n_registers(node, gen->prog);
  }
  ...
  if (isl_schedule_node_get_type(node) == isl_schedule_node_band) {
    isl_multi_val *unroll_factors;
    int tile_len = isl_schedule_node_band_n_member(node);
    assert (tile_len > 0);    
    assert (nregs > 0);

    printf("a1\n");
    unroll_factors = get_unroll_factors(node, tile_len, 0, gen, nregs);
    printf("a2\n");
	...
	isl_val *v = isl_multi_val_get_val(unroll_factors, i);
	nregs *= isl_val_get_num_si(v);
      }
      printf("a4\n");
      
      node = strip_mine_and_unroll(node, unroll_factors);
      node = isl_schedule_node_parent(node);
      printf("a5\n");
    }
    printf("a6\n");
  }
  printf("end unroll_nodes\n");
  return nregs;
}
	...
	__isl_take isl_schedule_node *node, void *user)
{
	...
	if (gen->wrap) {
	  char *key = (char *)calloc(500, sizeof(char));
	  snprintf(key, 500, "TS-%d:%d:", tile_len, n_coincident);
	  //snprintf(key, 500, "TS-%d:", tile_len);
	  isl_union_set *domain = isl_schedule_node_get_domain(node);
	  isl_union_set_foreach_set(domain, &get_stmt_str, key);
	  ...
	  struct ca_wrapper *wrap = gen->wrap;
	  //tile_size = wrap->traverse_tile(wrap->ant, wrap->s_path, wrap->node, 
	  //                                tile_size, tile_len, key, stmts, n_stmts);

	  int **outer_tile_size = wrap->traverse_tile(wrap->ant, wrap->s_path, wrap->node,
						     tile_size, n_coincident, key, stmts, n_stmts);

	  int check_size = 1;
	  for (i = 0; i < n_coincident; ++i) {
	    tile_size[i] = outer_tile_size[0][i];
	    check_size *= outer_tile_size[1][i];
	  }
	  assert (check_size <= 1024);
	  ...
	  if (!gen->sizes) {
	    isl_space *space = isl_space_params_alloc(gen->ctx, 0);
	    gen->sizes = isl_union_map_empty(space);
	  }
	  set_sizes(gen, "block", gen->kernel_id, outer_tile_size[1], n_coincident);
	}

	// Traverse Unroll Factors
	if (gen->wrap) {
	  struct ca_wrapper *wrap = gen->wrap;
	  int nregs = 0;
	  for (i = 0; i < isl_schedule_node_n_children(node); ++i) {
	    // call unroll nodes here
	    node = isl_schedule_node_child(node, i);
	    nregs += unroll_nodes(node, gen);
	    node = isl_schedule_node_parent(node);
	  }

	  if (n_coincident < tile_len) {	    
	    int *unroll_int = NULL;
	    isl_multi_val *unroll_factors = NULL;
	    printf("b1\n");
	    unroll_factors = get_unroll_factors(node, tile_len, n_coincident, gen, nregs);
	    printf("b2\n");
	    if (unroll_factors) {

	      unroll_int = (int *)calloc(tile_len - n_coincident, sizeof(int));
	      for (i = n_coincident; i < tile_len; ++i) {
		isl_val *v = isl_multi_val_get_val(unroll_factors, i);
		unroll_int[i - n_coincident] = isl_val_get_num_si(v);
	      }
	    ...
	    char *key = (char *)calloc(500, sizeof(char));
	    snprintf(key, 500, "ITS-%d:%d:", tile_len, n_coincident);
	    isl_union_set *domain = isl_schedule_node_get_domain(node);
	    isl_union_set_foreach_set(domain, &get_stmt_str, key);
	    ...
	    int *inner_sizes = wrap->traverse_inner_tile(wrap->ant, wrap->s_path, wrap->node, 
							 tile_len - n_coincident, unroll_int, key,
							 stmts, n_stmts);
	    ...
	    for (i = n_coincident; i < tile_len; ++i) {
	      tile_size[i] = inner_sizes[i - n_coincident];
	    }
	...
	free(tile_size);
	printf("end mark_outer_permutable\n");
	return node;
}
...
static int is_reducible(struct gpu_stmt stmt) {
  struct pet_stmt *pet = stmt.stmt;
  pet_expr *expr = pet_tree_expr_get_expr(pet->body);

  if (expr->type != pet_expr_op) {
    return 0;
  }
  
  if (expr->op != pet_op_add_assign) {
    return 0;
  }

  // check that the write array is only accessed once
  struct gpu_stmt_access *head, *trav;
  head = stmt.accesses;
  trav = head;
  ...
  while (trav != NULL) {
    if ((trav)->write) {
      write = isl_map_range(isl_map_copy((trav)->access));
      break;
    }
    trav = (trav)->next;
  }
  ...
  while ((trav) != NULL) {
    if (!(trav)->write) {
      isl_set *read = isl_map_range(isl_map_copy((trav)->access));
      if (isl_set_is_equal(read, write)) {
	trav = head;
	return 0;
      }
    }
 ...
 */
static __isl_give isl_schedule *compute_schedule(struct gpu_gen *gen, int **sccs)
{
	isl_schedule_constraints *sc;
	isl_schedule *schedule;
	int i;
	...
	if (gen->wrap) {
	  // mark reducible statements
	  isl_union_set *domain = isl_schedule_constraints_get_domain(sc);
	  isl_space *space = isl_union_set_get_space(domain);
	  
	  if (gen->wrap->reducible) {
	    isl_union_set_free(gen->wrap->reducible);
	  }

	  gen->wrap->reducible = isl_union_set_empty(space);
	
	  for (i = 0; i < gen->prog->n_stmts; ++i) {
	    if (is_reducible(gen->prog->stmts[i])) {
	      gen->wrap->reducible = isl_union_set_add_set(gen->wrap->reducible, 
							   gen->prog->stmts[i].stmt->domain);
	    }
	  }
	}
	schedule = isl_schedule_constraints_compute_schedule(sc, sccs, gen->wrap);
...
static __isl_give isl_schedule *compute_or_set_properties(void *user)
{
	...
	if (gen->options->reschedule)
	        return compute_schedule(gen, sccs);
	else
	...
	struct ppcg_options *options)
{
	...
	if (gen->wrap) {
	  struct ca_wrapper *wrap = gen->wrap;
	  int ntexs = 0;
	  char **texs = wrap->traverse_texture(wrap->ant, wrap->s_path, wrap->node, 
					       wrap->read_only, wrap->nread_only, &ntexs);
	  ...
	  for (i = 0; i < prog->n_array; ++i) {
	    if (exists(prog->array[i].name, texs, ntexs)) {
	      prog->array[i].texture = 1;
	    }
	  }

	  wrap->ant->ntexs = ntexs;
	  if (ntexs > 0) {
	    wrap->ant->texs = (char **)calloc(ntexs, sizeof(char *));
	    for (i = 0; i < ntexs; ++i) {
	      wrap->ant->texs[i] = strdup(texs[i]);
	    }
	  }
	  else {
	    wrap->ant->texs = NULL;
	...
	/*	if (gen->tex_array) {
	  ...
	  for (i = 0; i < prog->n_array; ++i) {
	    if (exists(prog->array[i].name, gen->tex_array, gen->n_tex_array)) {
	      prog->array[i].texture = 1;
	    }
	  }
...
{
	struct gpu_gen *gen = user;
	//p = isl_printer_print_str(p, "  cudaSetDevice(1);\n");
	p = isl_printer_print_str(p, "  signal(SIGINT, siginthandler);\n");
	return generate(p, gen, scop, gen->options);
}

static isl_stat my_constraint_fun(__isl_take isl_constraint *c, void *user) {
  isl_ctx *ctx = (struct isl_ctx*)user;
  isl_printer *p1;
  printf("cstr = ");
  p1 = isl_printer_to_file(ctx, stdout);
  isl_constraint_set_constant_si(c, -1);
  isl_printer_print_constraint(p1, c);
  printf("\n");

  return isl_stat_ok;
}

static isl_stat my_basic_map_fun(__isl_take isl_basic_map *map, void *user) {
  struct coding_ants *ca = (struct coding_ants*)user;
  isl_ctx *ctx = isl_basic_map_get_ctx(map);

  isl_printer *p1 = isl_printer_to_file(ctx, stdout);
  isl_printer_print_basic_map(p1, map);
  printf("\n");
}

static isl_stat my_fun(__isl_take isl_map *map, void *user) {
  struct coding_ants *ca = (struct coding_ants*)user;
  
  isl_map_foreach_basic_map(map, &my_basic_map_fun, ca);
}

static isl_stat my_fun2(__isl_take isl_map *map, void *user) {
  isl_ctx *ctx = (struct isl_ctx*)user;
  isl_space *space = isl_map_get_space(map);
  isl_local_space *ls = isl_local_space_from_space(space);

  isl_map_foreach_basic_map(map, &my_basic_map_fun, ctx);

  isl_printer *p1;
  p1 = isl_printer_to_file(ctx, stdout);
  printf("map = ");
  isl_printer_print_space(p1, space);
  printf("\n");
  return isl_stat_ok;
}
...
int generate_gpu(isl_ctx *ctx, const char *input, FILE *out,
	...
	__isl_give isl_printer *(*print)(__isl_take isl_printer *p,
		...
		struct gpu_types *types, void *user), void *user, struct coding_ants *ca)
{
	struct gpu_gen gen;
	int r;
	int i;
	...
	if (ca) {
	  if (!ca->init) {
	    gen.n_tex_array = ca->config.n_tex_array;
	    gen.tex_array = ca->config.tex_array;
	   
	    gen.wrap = ca->wrap;
	    gen.sccs = ca->config.sccs;
	  }
	  else {
	    // init
	    gen.edges = (struct simple_edges *)calloc(1, sizeof(struct simple_edges));
	  }
	}
	...
	if (ca) {
	  if (ca->init) {
	    ca->edges = gen.edges;
	    ca->n_stmts = isl_union_set_n_set(gen.prog->scop->domain);

	    // set the number of kernels
	    ca->n_kernels = gen.kernel_id;
	    
	    // set the number dimensions for each kernel
	    ca->n_tile_dims = (int *)calloc(ca->n_kernels, sizeof(int));
	    ca->n_grid_dims = (int *)calloc(ca->n_kernels, sizeof(int));
	    ca->n_block_dims = (int *)calloc(ca->n_kernels, sizeof(int));

	    isl_set *set;
	    for (i = 0; i < ca->n_kernels; ++i) {
	      set = extract_sizes(gen.used_sizes, "tile", i);
	      ca->n_tile_dims[i] = isl_set_n_dim(set);

	      set = extract_sizes(gen.used_sizes, "grid", i);
	      ca->n_grid_dims[i] = isl_set_n_dim(set);

	      set = extract_sizes(gen.used_sizes, "block", i);
	      ca->n_block_dims[i] = isl_set_n_dim(set);
	    }
	    isl_set_free(set);
	    
	    // get the number of non-scalar arrays
	    int n_array = 0;
	    for (i = 0; i < gen.prog->n_array; ++i) {
	      if (!gen.prog->array[i].read_only_scalar) {
		++n_array;
	      }
	    }

	    // allocate ca array
	    ca->arrays = (struct ca_array *)calloc(n_array, sizeof(struct ca_array));
	    ca->n_array = n_array;
		...
		continue;
	      ...
	      ca->arrays[ca_arr_ptr].name = strdup(gen.prog->array[i].name);
	      ca->arrays[ca_arr_ptr++].read_only = gen.prog->array[i].read_only;
	    }
	  }
	  else {
	    if (ca->wrap) {
	      ca->wrap->ant->nkernel = gen.kernel_id;
