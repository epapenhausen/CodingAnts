...
struct gpu_array_info {
	...
	int read_only_scalar;
        ...
        /* Is this a read-only array within the entire program? */
        int read_only;
	...
	int declare_local;
        ...
        /* Should the array be allocated in texture memory */
        int texture;
...
struct gpu_gen {
	...
	int kernel_id;
        ...
        /* Array of array names that should be put in texture memory */
        char **tex_array;
  
        /* Size of tex array */
        int n_tex_array;

        /* Partition of sccs selected by coding ants */
        int **sccs;

        /* A list of legality dependence edges. */
        struct simple_edges *edges;

        /* Wrapper containing the traversing ant and graph */
 ...
 *
 * reduction is set if the statement is copying to a reduction array.
 * This tells us we should initialize to zero instead since we will be 
 * using atomics to accumulate the final value.
 *
...
struct ppcg_kernel {
	...
	isl_union_set *arrays;
        isl_union_set *reduction_arrays;
