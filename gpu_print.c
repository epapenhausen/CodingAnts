	...
	__isl_take isl_printer *p, struct ppcg_kernel_stmt *stmt)
{
	...
	if (array->texture) {
	  assert(array->n_index <= 3);

	  if (array->n_index == 1) {
	    p = isl_printer_print_str(p, "tex1D<");
	  }
	  else if(array->n_index == 2) {
	    p = isl_printer_print_str(p, "tex2D<");
	  }
	  else {
	    p = isl_printer_print_str(p, "tex3D<");	  
	  }
	  p = isl_printer_print_str(p, array->type);
	  p = isl_printer_print_str(p, ">(");
	  p = isl_printer_print_str(p, array->name);
	  p = isl_printer_print_str(p, ", ");
	  isl_ast_expr_dump(index);
	  const int n_args = isl_ast_expr_get_op_n_arg(index);

	  for (i = 1; i < n_args; ++i) {
	    isl_ast_expr *res;

	    res = isl_ast_expr_get_op_arg(index, n_args - i);	    
	    p = isl_printer_print_ast_expr(p, res);
	    if (i < n_args - 1) {
	      p = isl_printer_print_str(p, ", ");	 
	    }
	  }
	...
	if (stmt->u.c.read) {
		...
		if (stmt->reduction) 
		  ...
		  p = stmt_print_global_index(p, stmt);
	        ...
	        if (stmt->reduction) {
	          p = isl_printer_print_str(p, "atomicAdd(&");
	        }

		int is_private = (stmt->atomic_store ? 
				  (strstr(isl_ast_expr_to_str(stmt->u.c.local_index), 
					  "private") != NULL) :
				  0);

		if (is_private) {
		  p = isl_printer_print_str(p, "atomicExch(&");
		}
		
		p = stmt_print_global_index(p, stmt);
		if (stmt->reduction || is_private)
		  p = isl_printer_print_str(p, ", ");
		else
		  p = isl_printer_print_str(p, " = ");
